...
public abstract class UITweener : MonoBehaviour
{
    ...
    void OnEnable()
    {
        if (style == Style.OnceEverytimeOnEnable)
        {
            ResetToBeginning();
        }
    }
    ...
    void Update ()
	{
		...
		if ((style == Style.Once || style == Style.OnceEverytimeOnEnable) && (duration == 0f || mFactor > 1f || mFactor < 0f))
		{
	...
	static public T Begin<T> (GameObject go, float duration) where T : UITweener
	{
		...
		if (comp == null)
		{
			...
			if (comp == null)
			{
				Debug.LogError("Unable to add " + typeof(T) + " to " + LogUtil.GetHierarchy(go), go);
				return null;
			}
		}
#endif
		comp.mStarted = false;
        comp.mDuration = 0; // Force to trigger the recalculation of 'mAmountPerDelta'
