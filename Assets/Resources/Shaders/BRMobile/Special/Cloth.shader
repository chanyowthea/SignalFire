...
SubShader{
	...
	uniform float _ClothExtraAmbient;
	...
	float4 frag(VertexOutput i) : COLOR{
		...
		float3 diffuse = (3 * NdotL - 2 * NdotL * NdotL) * texColor;

		float4 _SpecularmapGlossA_var = tex2D(_SpecularmapGlossA,i.uv0);
		float3 Specularmap = _SpecularColor.rgb * _SpecularmapGlossA_var.r * 2 * _SpecularIntensity;
		float3 specular = pow(max(0, dot(halfDirection, normalDirection)), max(_SpecularmapGlossA_var.b, 0.01) * 640.0) * Specularmap;
		/// Final Color:
		float3 finalColor = (_Ambientlight * LOBBY_AMBIENT_CLOTH + _ClothExtraAmbient + _SpecularmapGlossA_var.g) * texColor + (diffuse + specular) * attenColor;
		float4 finalRGBA = float4(saturate(finalColor),1);
	...
	Pass{
		...
		float4 frag(VertexOutput i) : COLOR{
			...
			float4 _SpecularmapGlossA_var = tex2D(_SpecularmapGlossA,i.uv0);
			float3 specular = pow(max(0,dot(halfDirection, normalDirection)), max(_SpecularmapGlossA_var.b, 0.01) * 640.0) * _SpecularColor.rgb * _SpecularmapGlossA_var.r * _SpecularIntensity;
...
SubShader{
	...
	float4 frag(VertexOutput i) : COLOR{
		...
		float3 texColor =  tex2D(_MainTex, i.uv0).rgb;
		float3 diffuse = (3 * NdotL - 2 * NdotL * NdotL) * texColor * (1.0f - _ShadowIntensiy);
		...
		/// Final Color:
		float3 finalColor = (_Ambientlight * INGAME_AMBIENT_CLOTH + _SpecularmapGlossA_var.g) * texColor + (diffuse + specular) * attenColor;
		float4 finalRGBA = float4(saturate(finalColor),1);
