		...
		void fogColor (Input IN, SURFACE_STRUCT o, inout fixed4 color) {
			...
			float3 wsCameraPos = _WorldSpaceCameraPos.xyz;// float3(_WorldSpaceCameraPos.x, _WorldSpaceCameraPos.y - (_FogData.y + 0.0001), _WorldSpaceCameraPos.z);
  			if (wsCameraPos.y<=_FogData.x || worldPos.y<=_FogData.x) {
				...
				float distanceToFog = 0.0;
				float3 fogCeilingCut = wsCameraPos;
				if (wsCameraPos.y > _FogData.x)
				{
					float h = min(wsCameraPos.y, _FogData.x);
					float xh = delta * (wsCameraPos.y - h);
					float2 xz = wsCameraPos.xz - ndirxz * xh;
					fogCeilingCut = float3(xz.x, h, xz.y);
					distanceToFog = distance(fogCeilingCut, wsCameraPos);
				}
				...
				if (distanceToFog<dist) {
					...
					if (fogLength>0) {
						...
						for (int k=DYNAMIC_FOG_STEPS;k>=0;k--, pos += st, distanceToFog += incDist) {
							...
							float fl = (distanceToFog - _FogData.z) / _FogData.y;
							fh = min(fh, fl);
						...
						fogColor *= _FogColor.a;
			 			color.rgb = color.rgb * (1.0 - fogColor.a) + fogColor.rgb;
					}
				}
	 		}
	    }
