Shader "BRMobile/VegetationGroup" {
	...
	Pass {  
		CGPROGRAM
			...
			v2f vert (appdata_t v)
			{
				...
				int id = floor(v.texcoord1.x + 0.0);
				vertex.x = dot(v.vertex, transformVec[id * 3]);
				vertex.y = dot(v.vertex, transformVec[id * 3 + 1]);
				vertex.z = dot(v.vertex, transformVec[id * 3 + 2]);

				vertex = vertex + float4(vertex.y * _Bias.xyz, 0.0f);
				vertex.xyz += vertex.y *  _WindVec.xyz;
				
				o.vertex = UnityObjectToClipPos(vertex);

				float3 worldPos = mul(unity_ObjectToWorld, vertex).xyz;
				o.worldUV.x = (worldPos.x - 470.0f) * 0.0005263f + 0.5f;
...
#include "BRMobileCommon.cginc"
	...
	fixed4 frag(v2f i) : SV_Target
	{
		fixed4 col = tex2Dbias(_MainTex, float4(i.texcoord.xy, 0.0f, -0.5f)) *_Color;
		col.rgb *= _Brightness * 0.8f;
		clip(col.a - _Cutoff);
