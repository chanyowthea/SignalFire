 ...
 Shader "Hidden/Dof/DepthOfField34" {
	...
	half4 fragDownsampleWithCocConserve(v2fDown i) : SV_Target {
		half2 rowOfs[2];   
  		...
  		rowOfs[1] = half2(0.0, _InvRenderTargetSize.y) * 2.0;  
		...
		half4 sampleC = tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(i.uv[0].xy + rowOfs[1], _MainTex_ST));
		half4 sampleD = tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(i.uv[1].xy + rowOfs[1], _MainTex_ST));
	...
	inline float LinearCameraDepth( float z )
	{
		return 1.0 / (-1499.5 * z + 1500.5);
	}
	...
	half4 fragDofApplyBg (v2fDofApply i) : SV_Target {		
		...
		float d = SAMPLE_DEPTH_TEXTURE (_CameraDepthTexture, UnityStereoScreenSpaceUVAdjust(i.uv.xy, _MainTex_ST));
		d = LinearCameraDepth (d);
