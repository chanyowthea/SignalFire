...
Shader "lg/Effect_2Tex_Dis_Additive_Depth01" {
    Properties {
        ...
        [MaterialToggle] _DisTex01_UVpan ("DisTex01_U/Vpan", Float ) = 0
        ...
        [MaterialToggle] _DisTex02_UVpan ("DisTex02_U/Vpan", Float ) = 0
        _DisTex02_Panner ("DisTex02_Panner", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _DisTex02_Panner;
            ...
            uniform fixed _DisTex02_UVpan;
            uniform fixed _DisTex01_UVpan;
            struct VertexInput {
...
////// Emissive:
                ...
                float2 angles = float2(_Tex01_UVAngle, _Tex02_UVAngle) * 3.141592654;
                float2 cosAngles = cos(angles);
                float2 sinAngles = sin(angles);

                float disTex01PanV = _Time.g * _DisTex01_PanV;
                float2 _DisTex01_UVpan_var = lerp(i.uv0 + float2(0, disTex01PanV), i.uv0 + float2(disTex01PanV,0), _DisTex01_UVpan);
                float4 _DisTex01_var = tex2D(_DisTex01,TRANSFORM_TEX(_DisTex01_UVpan_var, _DisTex01));
                float2 _DisTex01 = _DisTex01_var.rg * _DisTex01_Power;

                float node_4562 = _Time.g * _Tex01_PanV;
                float2 node_2921 = mul(lerp(i.uv0 +_DisTex01 + float2(node_4562, 0), i.uv0 + _DisTex01 + float2(0, node_4562), _Tex01_UVpan) - 0.5f,
                float2x2(cosAngles.x, -sinAngles.x, sinAngles.x, cosAngles.x)) + 0.5f;
                float4 _Tex01_var = tex2D(_Tex01, TRANSFORM_TEX(node_2921, _Tex01));

                float disTex02PanV = _Time.g * _DisTex02_Panner;
                float2 _DisTex02_UVpan_var = lerp(i.uv0 + float2(0, disTex02PanV), i.uv0 + float2(disTex02PanV,0), _DisTex02_UVpan);
                float4 _DisTex02_var = tex2D(_DisTex02, TRANSFORM_TEX(_DisTex02_UVpan_var, _DisTex02));
                float2 _DisTex02 = _DisTex02_var.rg * _DisTex02_Power;

                float node_2522 = _Time.g * _Tex02_PanV;
                float2 node_1670 = mul(lerp(i.uv0 +_DisTex02 + float2(node_2522, 0), i.uv0 + _DisTex02 + float2(0, node_2522), _Tex02_UVpan) - 0.5f,
                float2x2(cosAngles.y, -sinAngles.y, sinAngles.y, cosAngles.y)) + 0.5f;
                ...
                float3 emissive = _InstensityA * (_Tex01_Instensity * _Color_Tex01.rgb * _Tex01_var.rgb + _Tex02_Instensity * _Tex02_var.rgb * _Color_Tex02.rgb * _Color_Tex02.a) * _AlphaInstensityA * _Mask_Tex_var.r * (lerp(dot(_Tex01_var, 1), _Tex01_var.a, _Tex01_RGBAlpha) + lerp(dot(_Tex02_var, 1), _Tex02_var.a, _Tex02_RGBAlpha ))
